...
template is the source of truth for *displayed* data. They intentionally describe
the templates *as they render today*, including sections that currently show
fabricated fallback content. Those sections are flagged with
``fabricated_when_empty`` so a later phase can remove the fake-content dependency
without guessing which sections rely on it. See ``HARDCODED_CONTENT_AUDIT.md``.
...
PAGE_REQUIREMENTS: dict[str, list[dict[str, Any]]] = {
    "university": [
        _section("seo", "SEO & Meta",
                 optional_fields=["seo_title", "meta_description"],
        ...
        _section("hero", "Hero", required=True,
                 ...
                 always_rendered=True, fabricated_when_empty=True),
        ...
        _section("specializations", "Specializations Grid",
                 always_rendered=True, data_source=SOURCE_WORKSPACE),
    ...
    "course": [
        ...
        _section("hero", "Hero", required=True,
                 ...
                 optional_fields=["hero_description", "duration", "mode",
