...
(() => {
  ...
  function applyTemplateReplacements(tpl, site) {
    if (!tpl || !site) return tpl;
    
    // 1. Replace University Name
    const defaultNames = [
      "Sikkim Professional Online",
    ...
    for (const dName of defaultNames) {
      tpl = tpl.replace(new RegExp(dName, 'g'), site.name);
      const dSlug = dName.toLowerCase().replace(/\s+/g, '-');
      const activeSlug = site.name.toLowerCase().replace(/\s+/g, '-');
      tpl = tpl.replace(new RegExp(dSlug, 'g'), activeSlug);
    }

    // 2. Replace Email Addresses
    if (site.email) {
      const defaultEmails = [
        "admissions@nmimsonline.edu",
        "admissions.nmims@example.com",
        "admissions@example.com",
        "admissions@nmims.edu"
      ];
      for (const dEmail of defaultEmails) {
        tpl = tpl.replace(new RegExp(dEmail, 'g'), site.email);
      }
    }

    // 3. Replace WhatsApp Number
    if (site.whatsapp) {
      tpl = tpl.replace(/911800102513/g, site.whatsapp);
      tpl = tpl.replace(/wa\.me\/[0-9]+/g, `wa.me/${site.whatsapp}`);
    }

    // 4. Replace Phone Numbers
    if (site.phone) {
      tpl = tpl.replace(/1800-1025-136/g, site.phone);
    }

    // 5. Replace Address
    if (site.address) {
      tpl = tpl.replace(/V\.\s*L\.\s*Mehta\s*Road,\s*Vile\s*Parle\s*\(W\),\s*Mumbai\s*400056/g, site.address);
    }

    // 6. Replace logo letters
    tpl = tpl.replace(/(background:#FF5C35;color:#1C1B22;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:21px">)[A-Z]/g, `$1${site.name.charAt(0).toUpperCase()}`);
    tpl = tpl.replace(/(background:#FF5C35;color:#1C1B22;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:19px">)[A-Z]/g, `$1${site.name.charAt(0).toUpperCase()}`);

    // 7. Dynamic theme primary color mapping (replace #6B4FC9 with resolved color)
    if (site.branding && site.branding.primary_color) {
      tpl = tpl.replace(/#6B4FC9/g, site.branding.primary_color);
    }

    // 8. Lead URL parameter mapping
    if (site.site_id) {
      tpl = tpl.replace(/uni=[a-zA-Z0-9_-]+/g, `uni=${site.site_id}`);
    }

    return tpl;
  }

  async function handleEnquirySubmit(btn) {
    if (btn.disabled || btn.getAttribute('data-submitting') === 'true') {
      return;
    }
    ...
    const programSelect = container.querySelector('select');
    
    const name = nameInput ? nameInput.value.trim() : '';
    const email = emailInput ? emailInput.value.trim() : '';
    const phone = phoneInput ? phoneInput.value.trim() : '';
    const program = programSelect ? programSelect.value.trim() : '';
    ...
    const payload = {
      ...
      phone,
      email,
      program: program && program !== 'Select a program' ? program : 'General Enquiry',
      message: 'Submitted via dynamic enquiry widget',
    ...
    try {
      ...
      btn.style.backgroundColor = '#1a9d57';
      btn.style.color = '#fff';
  ...
  // Setup click delegation for enquiry form submissions
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('button');
    if (btn && btn.textContent.trim() === 'Submit Enquiry') {
      e.preventDefault();
      handleEnquirySubmit(btn);
    }
  });
  ...
  function boot(runtime, doc = document) {
    ...
    if (window.siteConfig) {
      parsed.template = applyTemplateReplacements(parsed.template, window.siteConfig);
      
      // Update page title
      const defaultNames = ["Sikkim Professional Online", "Sikkim Professional University", "Test-1 University", "Nodia University", "Chandigarh University", "Test 1"];
      for (const dName of defaultNames) {
        if (doc.title.includes(dName)) {
          doc.title = doc.title.replace(new RegExp(dName, 'g'), window.siteConfig.name);
        }
      }
      
      // Update Favicon if provided
      if (window.siteConfig.branding && window.siteConfig.branding.favicon) {
        const fav = doc.querySelector('link[rel*="icon"]');
        if (fav) fav.href = window.siteConfig.branding.favicon;
      }
    }
    ...
    fetch(location.href)
      ...
      .then((t) => {
        ...
        if (raw?.template) {
          if (window.siteConfig) {
            raw.template = applyTemplateReplacements(raw.template, window.siteConfig);
          }
  ...
  function init() {
    ...
    async function loadSiteConfig() {
      ...
      try {
        const res = await fetch(`${apiBase}/public/site?domain=${encodeURIComponent(host)}`);
        if (!res.ok) throw new Error("Status " + res.status);
        return await res.json();
      } catch (e) {
        console.warn("Failed to load site settings, falling back to local defaults:", e);
        return {
          site_id: "unknown",
          name: document.title.split(' ')[0] || "University",
          branding: {
